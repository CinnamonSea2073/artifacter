...
def culculate_op(data: dict):
    ...
    with codecs.open(f'{cwd}/data/Assets/duplicate.json', 'r', encoding='utf-8') as f:
        dup = json.load(f)
    with codecs.open(f'{cwd}/data/Assets/subopM.json', 'r', encoding='utf-8') as f:
        mapping = json.load(f)
...
def generation(data):
    ...
    def config_font(size): return ImageFont.truetype(
        f'{cwd}/data/Assets/ja-jp.ttf', size)

    Base = Image.open(f'{cwd}/data/Base/{element}.png').convert("RGBA")

    # キャラクター
    CharacterCostume = CharacterData.get('Costume')
    if CharacterName in ['蛍', '空']:
        CharacterImage = Image.open(
            f'{cwd}/data/character/{CharacterName}({element})/avatar.png').convert("RGBA")
    else:
        if CharacterCostume:
            CharacterImage = Image.open(
                f'{cwd}/data/character/{CharacterName}/{CharacterCostume}.png').convert("RGBA")
        else:
            CharacterImage = Image.open(
                f'{cwd}/data/character/{CharacterName}/avatar.png').convert("RGBA")

    Shadow = Image.open(
        f'{cwd}/data/Assets/Shadow.png').resize(Base.size)
    ...
    if CharacterName == 'アルハイゼン':
        CharacterAvatarMask2 = Image.open(
            f'{cwd}/data/Assets/Alhaitham.png').convert('L').resize(CharacterImage.size)
    else:
        CharacterAvatarMask2 = Image.open(
            f'{cwd}/data/Assets/CharacterMask.png').convert('L').resize(CharacterImage.size)
    ...
    # 武器
    Weapon = Image.open(
        f'{cwd}/data/weapon/{WeaponName}.png').convert("RGBA").resize((128, 128))
    ...
    WeaponRImage = Image.open(
        f'{cwd}/data/Assets/Rarelity/{WeaponRarelity}.png').convert("RGBA")
    ...
    # 天賦
    TalentBase = Image.open(f'{cwd}/data/Assets/TalentBack.png')
    ...
    for i, t in enumerate(['通常', 'スキル', "爆発"]):
        ...
        Talent = Image.open(
            f'{cwd}/data/character/{CharacterName}/{t}.png').resize((50, 50)).convert('RGBA')
    ...
    # 凸
    CBase = Image.open(
        f'{cwd}/data/命の星座/{element}.png').resize((90, 90)).convert('RGBA')
    Clock = Image.open(
        f'{cwd}/data/命の星座/{element}LOCK.png').resize((90, 90)).convert('RGBA')
    ...
    for c in range(1, 7):
        if c > CharacterConstellations:
            ...
            CharaC = Image.open(
                f'{cwd}/data/character/{CharacterName}/{c}.png').convert("RGBA").resize((45, 45))
    ...
    FriendShipIcon = Image.open(
        f'{cwd}/data/Assets/Love.png').convert("RGBA")
    ...
    for k, v in CharacterStatus.items():
        ...
        try:
            ...
            try:
                opicon = Image.open(
                    f'{cwd}/data/emotes/{k}.png').resize((40, 40))
    ...
    BaseAtk = Image.open(
        f'{cwd}/data/emotes/基礎攻撃力.png').convert("RGBA").resize((23, 23))
    ...
    if WeaponSubOPKey != None:
        BaseAtk = Image.open(
            f'{cwd}/data/emotes/{WeaponSubOPKey}.png').convert("RGBA").resize((23, 23))
        BaseAtkmask = BaseAtk.copy()
        Base.paste(BaseAtk, (1600, 155), mask=BaseAtkmask)

        if WeaponSubOPKey in disper:
            text = f'{optionmap.get(WeaponSubOPKey, WeaponSubOPKey)}  {WeaponSubOPValue}%'
        else:
            text = f'{optionmap.get(WeaponSubOPKey, WeaponSubOPKey)}  {WeaponSubOPValue}'
        D.text((1623, 155), text, font=config_font(23))
    ...
    if ScoreTotal >= 220:
        ScoreEv = Image.open(f'{cwd}/data/artifactGrades/SS.png')
    elif ScoreTotal >= 200:
        ScoreEv = Image.open(f'{cwd}/data/artifactGrades/S.png')
    elif ScoreTotal >= 180:
        ScoreEv = Image.open(f'{cwd}/data/artifactGrades/A.png')
    else:
        ScoreEv = Image.open(f'{cwd}/data/artifactGrades/B.png')
    ...
    for i, parts in enumerate(['flower', "wing", "clock", "cup", "crown"]):
        ...
        Preview = Image.open(
            f'{cwd}/data/Artifact/{details["type"]}/{parts}.png').resize((256, 256))
        ...
        Pmask = Image.open(
            f'{cwd}/data/Assets/ArtifactMask.png').convert('L').resize(Preview.size)
        ...
        MainIcon = Image.open(
            f'{cwd}/data/emotes/{mainop}.png').convert("RGBA").resize((35, 35))
        ...
        for a, sub in enumerate(details['sub']):
            ...
            SubIcon = Image.open(
                f'{cwd}/data/emotes/{SubOP}.png').convert("RGBA").resize((30, 30))
        ...
        if Score >= PointRefer[parts]['SS']:
            ScoreImage = Image.open(f'{cwd}/data/artifactGrades/SS.png')
        elif Score >= PointRefer[parts]['S']:
            ScoreImage = Image.open(f'{cwd}/data/artifactGrades/S.png')
        elif Score >= PointRefer[parts]['A']:
            ScoreImage = Image.open(f'{cwd}/data/artifactGrades/A.png')
        else:
            ScoreImage = Image.open(f'{cwd}/data/artifactGrades/B.png')
...
def get_character_discord_file(data) -> tuple[File, str]:
